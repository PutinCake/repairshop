...
export default function TicketTable({ data }: Props) {
    ...
    // Polling should be active only when the user is NOT searching.
    const isSearching = searchParams.has("searchText") && !!searchParams.get("searchText")
    // The hook expects a boolean to determine if it should be active.
    usePolling(!isSearching, 300000)
    ...
    const handlePageChange = (newPageIndex: number) => {
        // table.setPageIndex(newPageIndex) // This is not strictly needed when state is controlled
        const params = new URLSearchParams(searchParams.toString())
